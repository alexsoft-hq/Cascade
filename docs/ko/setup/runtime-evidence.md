...
- **애플리케이션 자신의 로그**. Java 에이전트가 `logging-otlp` 로 내보낼 때 나오는
  ...
  하나입니다. 그 앞에는 로거가 붙인 접두사가 있고, 앱이 찍은 다른 모든 줄
  사이에 섞여 있습니다.
...
않았습니다. 그 실행이 이 레시피와 다른 점이 하나 있습니다. 두 홉이 `RUNTIME_ONLY` 로
돌아온 이유가 바로 그것이고, 5 단계에서 밝혀 둡니다.
...
**private 헬퍼**(`findPaginatedForOwnersLastName`, `findPaginated`)를 거치는데,
이번 캡처는 그 헬퍼를 계측하지 않았습니다. 그래서 트레이스에는 컨트롤러가
리포지터리 바로 위에 있는 것으로 보였습니다. 레인은 본 그대로만 기록했고 정적
엣지를 지어내지 않았습니다.
...
있습니다. 관측은 그 엣지들에 붙고, 레인이 더할 홉은 남지 않습니다.
