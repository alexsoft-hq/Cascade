...
`<form>` 이 라우트로 전송하고, 링크가 다른 라우트를 엽니다. 이 레인은 그 페이지도
읽습니다. 프로파일이 이름을 댄 **템플릿 루트**에서입니다.
...
`<property name=… value=…/>` 자식으로 쓴 속성을 똑같이 읽고, 주석 처리된 빈은
빈이 아니며, prefix 도 suffix 도 없는 리졸버(`BeanNameViewResolver`)는 뷰 이름을
디렉터리가 아니라 빈으로 해석하므로 어떤 루트도 이름 대지 않습니다.
...
에서 루트를 읽습니다. 템플릿 루트는 pack 의 내용을 정하는 것이고, pack 이 아무도
...
- **인라인 `<script>` 블록.** 템플릿 자신의 지시자를 먼저 **무력화**합니다.
  ...
  `<spring:url>` 은 애플리케이션 루트에서 쓴 자기 `value` 가 되고(값 안의
  표현식은 구멍입니다), 닫는 태그와 제어 태그(`c:if`, `c:forEach`, `c:choose`,
  `c:when`, `c:otherwise`, `c:set` 같은 것)는 아무것도 남기지 않고, 나머지 태그는
  값 자리표가 됩니다. 셋 다 따옴표를 남기지 않습니다. 예전에는 그 따옴표가
  JavaScript 문자열을 일찍 끝내 버렸습니다.
...
- **링크.** 앱 루트에서 시작하는 경로를 가리키는 `href` 나 `th:href` 입니다.
  ...
  `/css`, `/js`, `/images`, `/fonts`)와 확장자로 빼고, 질의 문자열은 라우트의
  일부가 아니므로 떼며, 호스트가 붙은 주소는 남의 것입니다.
...
바로 그 규칙을 지납니다. 태그를 채워 넣고, 앱 자신의 표현식(`${…}`)은 원래대로
구멍이며, 질의 문자열은 떼고, 정적 자산은 라우트가 아닙니다.
...
소스가 어떤 라우터도 이름 대지 않는 동봉 루트도 그대로 읽으며, 두 줄 모두 그
사실을 말하지 조용히 넘어가지 않습니다. `init` 줄에는
...
`location.href` 는 GET 요청입니다" 를 보십시오.
...
| 증거 | 등급 | 왜 |
|---|---|---|
| 라우트를 선언하는 OpenAPI 문서 | 선언으로서 **EXACT** | 문서는 그 라우트가 존재한다는 프로젝트 자신의 진술이므로, 엔드포인트 노드는 **그것에 대해서만** 정확하고 다른 것에 대해서는 아닙니다. 코드도 서빙하는 라우트는 서로 뒷받침하며 코드 레인이 준 등급을 유지하고, 문서만 이름 대는 라우트는 **핸들러 엣지를 얻지 못하므로** 프런트엔드 호출이 엔드포인트에 닿고 거기서 멈추며 `code` 축이 그 이유로 `degraded` 가 됩니다. 아래 *OpenAPI 문서* 를 보십시오 |
...
| 브라우저 기록(HAR) | **RUNTIME_ONLY** | 기록은 요청이 한 번 일어났음을 증명할 뿐 코드가 무엇을 할 수 있는지는 증명하지 않으므로, 엣지는 모든 질의 모드의 하한 아래에 있습니다. **보여 주고**(`observed: true`) **절대 걷지 않으며**, 옆의 정적 엣지의 등급을 올리지도 않습니다. APM 트레이스나 액세스 로그는 여전히 읽지 않습니다. 아래 *기록(HAR)* 을 보십시오 |
...
경로를 서빙할 때 답이 올바른 프로젝트로 넘어가게 해 주는 것입니다
([개념](../concepts.md) 의 "경계 넘기" 를 보십시오).
...
`if` 가 `t` 를 다시 대입하는 것은 실제 코드이고, 초기화 값이 그 이름이 들고 있는
것은 아니기 때문입니다.
...
경로 별칭입니다. 그것들은 자기가 쓰인 파일이 아니라 **패키지**를 서술하므로 어떤
파일의 shard 도 그것을 정직하게 담을 수 없고, 프런트엔드가 보내는 모든 URL 을
다시 빚으므로 낡은 사본으로 답하는 것이 이 레인이 저지를 수 없는 유일한 실수
입니다. 모든 실행이 `node adapters/web/webfacts.mjs --configs-only …` 로 그것들을
다시 읽습니다. 소스 파일은 하나도 걷지 않고 프로세스 하나 띄우는 비용이 듭니다.
...
이것이 사 주는 불변식이 I-9 입니다. **같은 트리에서 증분 실행의 pack 다이제스트는
...
나쁩니다. 문서를 JSON 으로 바꾸거나, 그 구문 없이 라우트를 쓰십시오.
