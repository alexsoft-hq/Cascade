...
`endpoint → controller → service → mapper` 를 잇습니다. SQL 로 쓰이지 않고
`@Entity` 나 Spring Data 로 선언된 영속성도 함께 다룹니다. **웹 레인**(동봉된 파서, 설치
없음)은 엔드포인트 위쪽의 코드, 즉 프런트엔드 호출과 화면이 되는 라우터 선언을
읽습니다.

## 정직성 계약

모든 답은 네 개의 필드를 달고 다닙니다. 직렬화 전에 이 필드들을 검사합니다. 비슷하게 생긴
...
에서 DB 컬럼까지의 왕복을 찾습니다. Cascade 는 모든 엣지에 등급까지 답니다.
빌드 한 번 없이 소스만 읽어서 답합니다. 어떤 SAST 나 의존성 스캐너도 그 왕복을 답하지
않습니다. 소스만으로 호출이 확정되지 않는 곳은 엣지를 지어내는 대신 그렇다고
표시합니다. 보정할 수 있는 답이라야 에이전트가 손댈 수 있는 답이기 때문입니다.
...
아래 두 쪽은 영어 원문만 있습니다. 내용의 대부분이 영어 식별자로 된 표입니다.
