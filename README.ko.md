...
증명한 것보다 더 주장하지 않습니다. 그래프의 모든 엣지는 등급을 달고 있어서, 답
안에서 무엇이 증명된 부분이고 무엇이 후보 집합인지 구분됩니다. 만들 수 없었던
축은 빈 배열이 아니라 **선언**으로 돌아옵니다. 런타임 와이어링, 리플렉션, AOP
프록시는 파서가 볼 수 있는 범위 밖이므로 그래프에 아예 들어가지 않습니다.

### 주장하지 않는 것

SAST 와 CodeQL 은 취약점을 찾고, 이 도구는 변경이 어디까지 닿는지를 찾습니다.
그 둘은 이 페이지의 질문에 답하지 않고, 이 도구는 그 둘의 질문에 답하지
않습니다.

SAST 나 CodeQL 의 대체품이 아닙니다. "완전한" 영향 분석이 아닙니다. "안전한
리팩터링 보장"이 아닙니다. 언어 중립도 아니고 100% 정확하지도 않습니다. 이
도구가 내놓는 것은 측정된 하한값이며, 위의 어느 것도 정직하게 주장할 수
없습니다.
