...
컬럼 하나를 바꿨을 때 나오는 답을, 양방향으로 실제 오픈소스 프로젝트 두 곳에서
뽑아 봤습니다. 아래 정지 화면은 같은 뷰를 하나씩 다시 짚습니다.
...
에이전트가 코드를 고치기 전에 미리 물어볼 수 있습니다. 같은 답을 로컬 뷰어에도
그려 주므로 사람이 직접 읽을 수도 있습니다.
...
확인해야 하는지 압니다. 바로 이 지점 덕분에 스스로 코드를 고치는 도구에게도
영향도 그래프를 안심하고 넘길 수 있습니다. 파서가 볼 수 없는 것은 지어내지 않고
없다고 표시합니다. 런타임 와이어링, 리플렉션, AOP 프록시가 그렇습니다. 이
보정(calibration)이 곁다리가 아니라 제품 그 자체입니다.

### 어디에 쓰는가

SAST 와 CodeQL 은 취약점을 찾습니다. Cascade 는 변경이 어디까지 닿는지, 화면
에서 DB 컬럼까지 오가는 길을 찾습니다. 게다가 모든 엣지에 등급까지 답니다.
빌드는 한 번도 하지 않고 소스만 읽어서 답합니다. 어떤 SAST 나 의존성 스캐너도
그 왕복을 답하지 않습니다. 소스만으로 호출이 확정되지 않는 곳은 엣지를 지어내는
대신 그렇다고 표시합니다. 보정할 수 있는 답이라야 에이전트가 손댈 수 있는
답입니다. 그게 핵심입니다.

## 무엇에 답하는가

질문 여섯 개와 각각에 답하는 도구입니다. 전체 목록은 `tools/list` 가 알려 주며,
아래 여섯 개부터 먼저 익힐 만합니다.
...
이 문서에 실은 모든 출력 발췌에는 편집 규칙을 하나 똑같이 적용했습니다. 엔진이
출력한 절대 경로는 여러분이 직접 입력하게 될 상대 경로나 `~` 로 시작하는
형태로 바꿔 적었습니다. 그 밖에는 어떤 발췌도 손대지 않았습니다.

## 내 프로젝트에 10분

이 문서에 나오는 모든 숫자는 바로 아래 실행에서 나왔습니다. 커밋 `0504e86b` 의
...
디렉터리에는 여러분의 SQL 텍스트와 컬럼 주석이 들어가기 때문입니다. 동시에 홈
레지스트리에도 한 줄이 추가되므로, 이후 명령에서는 경로 대신 `--project mall`
로 부를 수 있습니다.
...
- **`trust`** 는 **계산된** 신뢰 수준이며 사람이 타이핑해 넣은 값이 아닙니다.
  값은 네 가지입니다. `UNCERTIFIED` 는 점수를 매긴 것이 없다는 뜻이고, 여기서는
  이 프로젝트에 승인된 골든 코퍼스가 없다는 말을 숨기지 않고 그렇게 합니다.
  `GOLDEN_FAIL` 은 검사 하나가 실제로 틀렸다는 뜻입니다. `RUNTIME_PASS` 는
  점수를 매긴 검사는 모두 통과했고 그중 적어도 하나는 프로그램이 도는 기록에서
  라벨을 받았다는 뜻입니다. 즉 실제로 실행된 것은 답이 덮고 있다는 것까지만
  말하고, 정밀도는 다루지 않습니다. `GOLDEN_PASS` 는 나머지 하나입니다.
  `knownGaps` 는 degraded 이거나 아예 만들어지지 않은 축을 하나씩 이름으로
  밝힙니다.
...
영향받지 않습니다. 위 엔드포인트가 `EXACT` 인 이유는 구체 컨트롤러 메서드의
매핑 애너테이션이 그 핸들러 **그 자체**이기 때문입니다. 동시에 `PROVISIONAL`
인 이유는 인증된 그 무엇도 아직 그 메서드를 본 적이 없기 때문입니다.
...
**1 초 규칙.** 이 루프는 편집과 다음 질문 사이의 짧은 틈에 들어가야만 쓸모가
있습니다. 그래서 오버레이에는 1 초 예산이 있습니다. 위 실행은 519 개 파일의
백엔드와 128 개 파일의 프런트엔드를 상대로 그중 311 ms 를 썼습니다. 실행
중에는 아무것도 기록하지 않습니다. pack 도, 팩트 캐시도 쓰지 않습니다.
오버레이는 과대 근사할 수는 있어도 빠뜨릴 수는 **없습니다.** 이 말은 문서 속
...
"첫 번째 것을 고른다" 같은 폴백은 없습니다. 엉뚱한 프로젝트에 자신 있게 답하는
것, 이 도구는 바로 그 실패를 막으려고 만들어졌기 때문입니다.
...
`project` 가 붙습니다. `basis.siblings` 에는 걸어 다닌 pack 마다 자기
다이제스트가 실립니다. `answer.federation` 에는 건넌 곳, 아무 프로젝트도
서빙하지 않는 호출(고치는 법은 그 프로젝트를 등록하는 것), 라우트 색인이 없어
건너뛴 프로젝트가 적힙니다. 이어 붙이는 일은 질문할 때 일어납니다. `analyze` 가 pack
옆에 써 두는 작은 `routes.json` 을 읽을 뿐, pack 과 다이제스트는 바뀌지
않습니다. 건너는 엣지는 잘해야 SOUND_SET 입니다. 여러 프로젝트가 같은 라우트를
서빙하면 HEURISTIC 이 됩니다. 그래도 지어내지는 않습니다. 아무도 서빙하지 않는 호출은
지금처럼 "pack 을 떠남"으로 남습니다. `federate: false` 면 pack 하나만으로
답합니다. 프로젝트 전체를 그리는 세 화면도 같은 규칙입니다. Overview 는
연결된 프로젝트와 그리로 가는 라우트를 나열합니다. Graph 는 요청이 형제
프로젝트에서 닿는 것을 이 프로젝트 지도 옆에 묶음으로 그립니다. ERD 는 형제의
...
**격자는 원소가 하나인 후보 집합도 절대 `EXACT` 로 올리지 않습니다.** 좁히는 것은
...
위 [설치](#설치) 에서 보였습니다. 빠진 준비물을 명령마다 하나씩 찾아내는 대신,
한 번의 보고서로 받으려고 만들었습니다.
...
참여합니다. 나머지 44 개는 버리지 않고, 지도 아래 띠에 이름만 적어 둡니다.
...
발견 대상에서 뺐습니다. 기록이란 의도적으로 만드는 것입니다. 트리에 우연히
있다는 이유로 주워 오면, 무관한 캡처가 이 pack 이 무엇을 관측했다고 주장할지를
정해 버리기 때문입니다.
...
얻습니다. 관측되지 않은 후보는 있던 그대로 남습니다. 어떤 정적 규칙으로도
설명되지 않는 홉은 보여 주기만 하고 걷지 않는 `RUNTIME_ONLY` 엣지가 됩니다.
아무것도 승격되지 않습니다. SQL 텍스트나 바인딩된 파라미터는 pack 에 들어가지
않습니다. 발견 단계도 없습니다. 파일은 OTLP/JSON 문서여도 되고, OpenTelemetry Java 에이전트가
...
적게 닿으면 실패**합니다. 값이 올라가도, 누군가 `--accept` 를 돌려 기준선을
다시 쓰고 그 차이를 출력하기 전까지는 아무것도 바뀌지 않습니다. 조용히 좋아진 숫자는
...
선택적인 네이티브 분석 레인은 각자의 라이선스 아래 별도 서브프로젝트로 살며,
Apache-2.0 코어에 링크되지 않습니다. 예를 들면 앞으로 LGPL 솔버를 쓰는 JVM
데이터플로 레인 같은 것입니다. 그 경계는 `NOTICE` 에 전부 적혀 있습니다.
