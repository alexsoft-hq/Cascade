...
export const VIEWER_STRINGS = {
  en: {
    ...
    'hint.compare.lead': 'What changed between this pack and another one of the same project.',
    'hint.compare.more': 'Serve the pack of the main branch and the pack of a change under two ids, open the change, and pick the main branch here. Read the first panel before anything else: a difference is a code change only when both packs were analyzed the same way. A renamed method shows as one removal and one addition.',
    'compare.base.label': 'compare this project against',
    'compare.base.title': 'the other pack this server serves: it is the base, and the project on screen is the head',
    'compare.none': 'this server serves no other pack to compare with',
    'compare.loading': 'comparing the two packs…',
    'compare.base': 'base',
    'compare.head': 'head',
    'compare.counts': 'nodes {nodes}, edges {edges}',
