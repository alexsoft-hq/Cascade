...
export const VIEWER_STRINGS = {
  en: {
    ...
    'hint.compare.lead': 'What changed in this project since an earlier build of it.',
    'hint.compare.more': 'Every certified analyze keeps the pack it replaces, and this tab compares the pack on screen with one of those. Read the first panel first: a difference is a code change only when both builds were analyzed the same way. A renamed method is one removal and one addition. For any other commit, `cascade diff --base-commit <rev>` builds the base.',
