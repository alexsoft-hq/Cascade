...
import {
  El, parseBody, themeTokens, bootPage as boot, settle, ev,
} from './helpers/viewer_page.mjs';
import { startViewer } from './helpers/viewer_fixtures.mjs';

// The PAGE's own behaviour (SPEC §15 M9), run for real: the viewer's scripts are
// evaluated in a node:vm against a small DOM stub and a REAL viewer server
// holding two projects. The stub and the loader live in
// test/helpers/viewer_page.mjs, so every viewer test loads the page the one
// way the browser does. No browser is started here — the picture itself
